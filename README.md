...
1조 그룹프로젝트. 왁뿌볼을 터뜨리고 키캡을 두드려 코인을 모으고, 뽑기와 상점으로 오브제를 수집하는 스트레스 해소 앱입니다.
...
| 핵심 루프 | 누르기(왁뿌볼·키캡) → 코인 획득 → 뽑기·구매로 오브제 수집 → 컬렉션 완성 |
...
# 왁뿌볼 크랙 프레임 등록 (PNG → WebP, 프레임 수 = 등급별 타격 수 10/15/20/20, 01부터 빠짐없이)
pip install pillow
...
│  │  └─ crack/<id>/          # 왁뿌볼 크랙 프레임 01~NN.webp (타격당 1장)
...
└─ src/
   ...
   ├─ data/
   │  ├─ toys.json            # 오브제 48종 데이터(왁뿌볼 17 · 키캡 31)
...
| 왁뿌볼 룸 | `/wakpuball` | 연타로 파괴, 타격마다 크랙 프레임 전환(프레임 있는 오브제), 칠 때마다 코인 굴림, 디자인 변경 | `Wakpuball.jsx`, `WakpuStage.jsx`, `crackFrames.js` |
| 키캡 룸 | `/keycap` | 보유 디자인별 키 1개씩, 누르면 장착 + 고유 사운드 + 코인, ESC 키 지원, ASMR 토글 | `Keycap.jsx`, `sound.js` |
| 뽑기 | `/gacha` | 코인 200개로 유료 등급 랜덤 1개(캡슐 흔들림 녹음 약 2초 재생), 중복 시 30% 환급, 광고 보고 5코인(mock) | `Gacha.jsx`, `CapsuleMachine.jsx` |
| 컬렉션 | `/collection` | 왁뿌볼/키캡 탭 → COMMON/RARE/PREMIUM/LIMITED 등급 탭별로 보유 오브제(장착) + 미획득 목록 | `Collection.jsx` |
| 시크릿 룸 | `/secret` | 시크릿 키 1개 = 히든 왁뿌볼 룸 또는 히든 키캡 룸 중 하나 1회 입장, 키 없으면 잠김 | `Secret.jsx` |
...
| grade | tier | 획득 방법 | 수량 | 왁뿌볼 파괴 타격 수 |
|---|---|---|---|---|
| COMMON | free | 처음부터 보유 | 왁뿌볼 4 · 키캡 4 | 10회 (버블 젤리·선더 크랙은 15회) |
| RARE | paid | 뽑기 (코인 200개, 일반 확률) | 왁뿌볼 9 · 키캡 17 | 15회 |
| PREMIUM | premium | 상점 구매 (₩2,900~3,400, mock) | 왁뿌볼 2 · 키캡 4 | 20회 |
| LIMITED | paid | 뽑기 (코인 200개, 극악 확률 — 합산 왁뿌볼 1.1% · 키캡 1.7%) | 왁뿌볼 2 · 키캡 6 | 20회 |

왁뿌볼 파괴 타격 수는 등급 기준 통일(키캡 게이지는 오브제와 무관하게 동일 적용). 버블 젤리·선더 크랙은 RARE 시절 15장짜리 크랙 프레임으로 제작돼 COMMON으로 옮긴 뒤에도 15회 유지. 뽑기 확률 계산은 `src/data/gradeOdds.js`, 화면 표시는 `/gacha`에서 실제 값 그대로 확인 가능.
...
### 시크릿 키
- 획득: 상점 300코인 구매 / 광고 보고 받기 하루 최대 3개(mock) / 왁뿌볼·키캡 타격 1회마다 0.06% 확률로 코인 대신 드롭
- 사용: 키 1개 = 히든 왁뿌볼 룸 또는 히든 키캡 룸 중 하나 1회 입장(입장권), 키 없으면 시크릿 룸 잠김
...
| `crackFrames` | `{ dir, count }` 왁뿌볼 크랙 프레임 폴더·장수, 없으면 고정 이미지 + 조각 파편 연출 |
| `hitSound` | 왁뿌볼 타격 녹음 경로(`sounds/…wav`), 없으면 합성 크런치음 — 현재 COMMON 4종·RARE 9종·PREMIUM 2종 (LIMITED 2종은 hitSynth) |
...
| 왁뿌볼 단계별 파손 사진 | 완료 — 왁뿌볼 17종 전부 크랙 프레임 적용(`public/images/crack/<id>/`), 새 디자인은 `scripts/import_crack_frames.py`로 추가 |
